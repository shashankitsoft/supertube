...
async function fetchLatestVideo(channelHandle) {
  ...
  const liveUrl = `https://www.googleapis.com/youtube/v3/search?key=${apiKey}&channelId=${channelId}&part=snippet,id&eventType=live&type=video&maxResults=1`;
  const liveRes = await axios.get(liveUrl);
  const live = liveRes.data.items[0];
  const liveVideo = live ? `https://www.youtube.com/watch?v=${live.id.videoId}` : null;
...
(async () => {
  ...
  for (const category of channels) {
    for (const ch of category.channels) {
      try {
        const { latest, live, channelId } = await fetchLatestVideo(ch.handle);
        results.push({
